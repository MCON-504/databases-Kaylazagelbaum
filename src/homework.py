...
def student_grade_report(conn: sqlite3.Connection, student_id: int) -> list[sqlite3.Row]:
    ...
    cursor = conn.execute("""
        ...
        g.score,
        a.max_points,
        ROUND(1.0 * score / max_points * 100, 1) AS percent FROM grades g
...
def leaderboard(conn: sqlite3.Connection) -> list[sqlite3.Row]:
    ...
    cursor = conn.execute("""
        ...
        ROUND(AVG(.0 * g.score / a.max_points * 100), 1) AS avg_percent FROM students s
        JOIN grades g on g.student_id = s.id
        JOIN assignments a ON g.assignment_id = a.id
        GROUP BY s.id
        ORDER BY avg_percent DESC, student_name;""")
